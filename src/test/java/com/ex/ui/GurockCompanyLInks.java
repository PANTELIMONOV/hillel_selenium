...
public class GurockCompanyLInks extends BaseTest {
    ...
    @Test
    public void legalPolicesTab() {
        webDriver.get(homeUrl);
        WebElement webElement = webDriver.findElement(By.xpath("//a[contains(., 'Legal')]"));
        ...
        Assert.assertTrue(findElement(webDriver, By.xpath("//a[contains(.,'Data Storage Policy')]")).isDisplayed());
    }
    ...
    public void legalLink() {
        ...
        WebElement subscribeBtn = waitElementClickable(webDriver, By.xpath("//div[@class='col-6 col-sm-6 col-md-4 " +
                "col-lg-3 gk-footer-menu-item-first']/a"));
        scrollToElement(webDriver, subscribeBtn);
        subscribeBtn.click();

        actualUrl = webDriver.getCurrentUrl();

        Assert.assertEquals(actualUrl, homeUrl + "newsletter/");
    }
}
